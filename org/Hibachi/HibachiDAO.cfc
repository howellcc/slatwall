<cfcomponent output="false" accessors="true" extends="HibachiObject">
	...
	<cfscript>
		...
		public void function flushORMSession(boolean runCalculatedPropertiesAgain=false) {
			...
			var entitiesSynced = [];
			//get items that need to synced at the end of the request
			if(arraylen(modifiedEntities)){
				if(!arrayFind(entitiesSynced,entity.getPrimaryIDValue())){
					getService('HibachiEventService').announceEvent('entityQueuePushNow',{modifiedEntities=modifiedEntities});
					arrayAppend(entitiesSynced,entity.getPrimaryIDValue());
				}
			}

...
component accessors="true"{
	...
	*/
	...
	this.logLevels = new cachebox.system.logging.LogLevels();
	 ...
	 */
	function init( required cachebox.system.logging.config.LogBoxConfig config, coldbox="" ){
		// LogBox Unique ID
	 ...
	 *
	 * @config The LogBoxConfig object to use to configure this instance of LogBox: cachebox.system.logging.config.LogBoxConfig
	 * @config.doc_generic cachebox.system.logging.config.LogBoxConfig
	 */
	function configure( required config ){
		lock name="#variables.logBoxID#.logbox.config" type="exclusive" timeout="30" throwOnTimeout=true{
			...
			variables.loggerRegistry = {
				"ROOT" = new cachebox.system.logging.Logger( argumentCollection=args )
			};
		}
	}
	 ...
	 *
	 * @return cachebox.system.logging.Logger
	 */
	...
	function getLogger( required category ){
		...
		lock name="#variables.logboxID#.logBox.logger.#arguments.category#" type="exclusive" throwontimeout="true" timeout="30"{
			if( NOT structKeyExists( variables.loggerRegistry, arguments.category ) ){
				// Create logger
				var oLogger = new cachebox.system.logging.Logger( argumentCollection=args );
				// Inject Root Logger
